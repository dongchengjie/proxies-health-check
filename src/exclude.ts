...
let cachedExcludedKeys: Set<string> | undefined;

export const shouldExcludeProxy = (proxy: any, excluded: any[] = []) => {
  if (!excluded || excluded.length === 0) return false;

  if (!cachedExcludedKeys) {
    cachedExcludedKeys = new Set<string>();
    for (const item of excluded) {
      if (item[EXCLUDED_TIMES_KEY] >= inputs["max_excluded_times"]) {
        cachedExcludedKeys.add(uniqueKey(item));
      }
    }
  }

  return cachedExcludedKeys && cachedExcludedKeys.has(uniqueKey(proxy));
};
